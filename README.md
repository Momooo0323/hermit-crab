...
- **权限控制** — 10 项独立开关（文件读写删、Shell 命令、记忆管理、搜索、知识库、计划），图形化设置面板
...
- **Shell 命令执行** — `/exec` 执行系统命令，确认对话框 + 高危命令拦截 + 30s 超时，可通过 FC 自动调用
- **文件目录管理** — 创建/读取/写入/删除文件 + 创建文件夹（`/mkdir`），全部受权限控制
